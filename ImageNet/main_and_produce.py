...
from data.build_produce import build_loader, RealLabelsImagenet
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler):
    model.train()
    optimizer.zero_grad()
    pred_prob = torch.zeros(config.DATA.TRAIN_SIZE).cuda()
    accuracy = torch.zeros(config.DATA.TRAIN_SIZE).int().cuda()
    ...
    for idx, data in enumerate(data_loader):
        train_samples = data[0]['image']
        eval_samples = data[0]['eval_image']
        targets = data[0]['label'].squeeze(-1).long()
        indices = data[0]['global_index'].squeeze(-1).long()

        if mixup_fn is not None:
            train_samples, train_targets = mixup_fn(train_samples, targets)
        else:
        ...
        if config.MODEL.TYPE == 'resnet':  # Batch Norm
            model.eval()
        with torch.no_grad():
            with torch.cuda.amp.autocast(enabled=config.AMP_ENABLE):
                eval_outputs = model(eval_samples)

            prob = softmax(eval_outputs, dim=1).gather(dim=1, index=targets.unsqueeze(1)).squeeze(-1).float()
            pred_prob = pred_prob.scatter(dim=0, index=indices, src=prob)
    ...
    dist.all_reduce(accuracy, op=dist.ReduceOp.MAX)
    np.savetxt(config.OUTPUT + f'/pred_prob_history/prob_{epoch}.txt', pred_prob.cpu().numpy())
...
if __name__ == '__main__':
    ...
    os.makedirs(config.OUTPUT + '/pred_prob_history', exist_ok=True)
