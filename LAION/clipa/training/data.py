...
def expand_urls(urls, weights=None):
    ...
    if isinstance(urls, str):
        ...
        assert len(weights) == len(
            urllist), f"Expected the number of data components ({len(urllist)}) and weights({len(weights)}) to match."
...
class tarfile_to_samples_nothrow(wds.PipelineStage):
    def __init__(
            self,
            coreset_path=None,
            handler=log_and_continue
    ):
        self.coreset_path = coreset_path
        self.handler = handler

    def run(self, src):
        streams = url_opener(src, handler=self.handler)
        files = tar_file_expander(streams, handler=self.handler)
        samples = group_by_keys_nothrow(files, self.coreset_path, handler=self.handler)
        return samples
    ...
    def __init__(
        ...
        if self.weights is not None:
            assert len(self.urls) == len(
                self.weights), f"Number of urls {len(self.urls)} and weights {len(self.weights)} should match."
...
def get_wds_dataset(args, preprocess_img, is_train, epoch=0, floor=False, tokenizer=None):
    ...
    if is_train:
        ...
        pipeline.extend([
            ...
            tarfile_to_samples_nothrow(coreset_path=args.coreset),  # wds.tarfile_to_samples(handler=log_and_continue),
            wds.shuffle(
